    ...
    public static function ajaxData(): void
    {
        ...
        $rows = $wpdb->get_results(
            "SELECT p.id AS politician_id, p.full_name, p.position, p.party, p.age, p.biography, p.career_history,
                    p.municipality_history, p.phone, p.email, p.advisors, p.photo_url, p.source_url, p.source_name, p.data_status,
        ...
        foreach ($rows as $row) {
            $entries[] = [
                ...
                'photo_url' => !empty($row['photo_url']) ? (string) $row['photo_url'] : null,
                'location' => [
