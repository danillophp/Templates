...
class MapaPoliticoAdmin
{
    public static function init(): void
    {
        ...
        add_action('wp_ajax_mapa_politico_ai_enqueue_all', [self::class, 'ajaxAiEnqueueAll']);
        add_action('wp_ajax_mapa_politico_ai_enqueue_one', [self::class, 'ajaxAiEnqueueOne']);
        add_action('wp_ajax_mapa_politico_ai_process_next', [self::class, 'ajaxAiProcessNext']);
    }
        ...
        <div class="wrap">
            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" enctype="multipart/form-data">
                ...
                <table class="form-table" role="presentation">
                    ...
                    <tr><th><label for="photo_url">Foto (URL externa)</label></th><td><input type="url" class="regular-text" id="photo_url" name="photo_url" value="<?php echo esc_attr($editing['photo_url'] ?? ""); ?>" placeholder="https://..."></td></tr>
    ...
    public static function saveEntry(): void
    {
        ...
        $photoUrl = esc_url_raw(wp_unslash($_POST['photo_url'] ?? ''));
        ...
        try {
            if ($politicianId > 0) {
                ...
                $politicianData['photo_url'] = $photoUrl;
    ...
    public static function renderAiSync(): void
    {
        ...
        $queueNonce = wp_create_nonce('mapa_politico_ai_queue_nonce');
        $queueRows = MapaPoliticoAI::getQueueRows(200);
        $municipalities = MapaPoliticoAI::getGoiasMunicipalities();
        ...
        <div class="wrap">
            ...
            <h2>Fila de sincronização por município</h2>
            <p>Processa <strong>1 município por execução</strong> (evita erro 500 e timeout no servidor).</p>
            <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
                <select id="mp-queue-municipality">
                    <option value="">Selecione um município</option>
                    <?php foreach ($municipalities as $m): ?>
                        <option value="<?php echo esc_attr((string) ($m['id'] ?? '')); ?>"><?php echo esc_html((string) ($m['nome'] ?? '')); ?></option>
                    <?php endforeach; ?>
                </select>
                <button type="button" class="button" id="mp-enqueue-one">Sincronizar município</button>
                <button type="button" class="button button-primary" id="mp-enqueue-all">Sincronizar todos</button>
                <button type="button" class="button" id="mp-process-next">Processar próximo da fila</button>
            </div>
            <p id="mp-queue-feedback"></p>

            <table class="widefat striped" id="mp-queue-table">
                <thead><tr><th>ID</th><th>Município</th><th>Código IBGE</th><th>Status</th><th>Tentativas</th><th>Erro</th><th>Atualizado</th></tr></thead>
                <tbody>
                <?php foreach ($queueRows as $q): ?>
                    <tr>
                        <td><?php echo esc_html((string) $q['id']); ?></td>
                        <td><?php echo esc_html((string) $q['municipality_name']); ?></td>
                        <td><?php echo esc_html((string) $q['municipality_code']); ?></td>
                        <td><?php echo esc_html((string) $q['status']); ?></td>
                        <td><?php echo esc_html((string) $q['attempts']); ?></td>
                        <td><?php echo esc_html((string) ($q['last_error'] ?? '')); ?></td>
                        <td><?php echo esc_html((string) $q['updated_at']); ?></td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
            </table>
        ...
        <script>
            (() => {
                ...
                const queueNonce = <?php echo wp_json_encode($queueNonce); ?>;

                const queueFeedback = document.getElementById('mp-queue-feedback');
                const municipalitySelect = document.getElementById('mp-queue-municipality');

                const sendQueueAction = async (action, extra = {}) => {
                    const body = new URLSearchParams({ action, nonce: queueNonce, ...extra });
                    ...
                    if (!response.ok || !data.success) throw new Error(data?.data?.message || 'Falha na fila');
                    queueFeedback.textContent = data?.data?.message || 'Ação executada';
                    window.location.reload();
                };

                document.getElementById('mp-enqueue-all')?.addEventListener('click', () => sendQueueAction('mapa_politico_ai_enqueue_all'));
                document.getElementById('mp-process-next')?.addEventListener('click', () => sendQueueAction('mapa_politico_ai_process_next'));
                document.getElementById('mp-enqueue-one')?.addEventListener('click', () => {
                    const code = municipalitySelect?.value || '';
                    if (!code) { queueFeedback.textContent = 'Selecione um município.'; return; }
                    sendQueueAction('mapa_politico_ai_enqueue_one', { municipality_code: code });
                });
    ...
    public static function runAiSync(): void
    {
        ...
        MapaPoliticoAI::enqueueAllMunicipalities();
        MapaPoliticoAI::processNextQueueItem();
        wp_safe_redirect(admin_url('admin.php?page=mapa-politico-ia&synced=1'));
        exit;
    }

    public static function ajaxAiEnqueueAll(): void
    {
        if (!current_user_can('manage_options')) {
            wp_send_json_error(['message' => 'Sem permissão.'], 403);
        }

        check_ajax_referer('mapa_politico_ai_queue_nonce', 'nonce');
        $result = MapaPoliticoAI::enqueueAllMunicipalities();
        wp_send_json_success(['message' => 'Municípios enfileirados: ' . (int) ($result['enqueued'] ?? 0)]);
    }

    public static function ajaxAiEnqueueOne(): void
    {
        if (!current_user_can('manage_options')) {
            wp_send_json_error(['message' => 'Sem permissão.'], 403);
        }
        ...
        check_ajax_referer('mapa_politico_ai_queue_nonce', 'nonce');
        $code = sanitize_text_field(wp_unslash($_POST['municipality_code'] ?? ''));
        $result = MapaPoliticoAI::enqueueMunicipalityByCode($code);
        if (!$result['ok']) {
    ...
    public static function ajaxAiProcessNext(): void
    {
        if (!current_user_can('manage_options')) {
            wp_send_json_error(['message' => 'Sem permissão.'], 403);
        }
        ...
        check_ajax_referer('mapa_politico_ai_queue_nonce', 'nonce');
        $result = MapaPoliticoAI::processNextQueueItem();
    ...
    private static function deletePoliticians(array $ids, string $scope): array
    {
        ...
        if ($scope === 'all') {
            $targets = $wpdb->get_results("SELECT id, location_id, photo_url FROM {$politiciansTable}", ARRAY_A);
        } else {
            ...
            $targets = $wpdb->get_results("SELECT id, location_id, photo_url FROM {$politiciansTable} WHERE id IN ({$in})", ARRAY_A);
        }
